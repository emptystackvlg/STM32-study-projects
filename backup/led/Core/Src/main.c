...
int main(void)
{
  ...
  HAL_Delay (100);
  lcd_gotoxy(&lcd1, 1, 0);
  lcd_puts(&lcd1, "Hello world_1");
  lcd_gotoxy(&lcd1, 1, 1);
  lcd_puts(&lcd1, "Hello world_2");
  lcd_gotoxy(&lcd1, 1, 2);
  lcd_puts(&lcd1, "Hello world_3");
  lcd_gotoxy(&lcd1, 1, 3);
  lcd_puts(&lcd1, "Hello world_4");
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  /*if (HAL_GetTick()-led_time >=500)
	  {
		  ...
		  lcd_gotoxy(&lcd1, 0, 1);
		  lcd_puts(&lcd1, str_0);
		  HAL_UART_Transmit(&huart2,str_0,sizeof(str_0), 100);
